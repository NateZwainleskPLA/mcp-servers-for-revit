...
.DESCRIPTION
    ...
    - Installs a local compiled build when one is available in this repo
    - Otherwise downloads the latest (or specified) pre-built Release from GitHub
    - Copies or extracts to the correct Addins folder
    - Unblocks all files so Windows does not prevent loading
    - Verifies all required files and dependencies are present
    - Optionally configures Claude Desktop for either the local server build or npm package
...
.PARAMETER Repo
    GitHub repository in owner/name form for release downloads.

.PARAMETER SourcePath
    Optional path to a local compiled add-in output containing the .addin file
    and revit_mcp_plugin folder. When omitted, the script auto-detects local
    builds under plugin/bin/ for the selected Revit version.

.PARAMETER ServerPath
    Optional path to a local built MCP server entry point (for example
    server/build/index.js). Used when configuring Claude Desktop for local builds.
...
.EXAMPLE
    .\install.ps1 -RevitVersion 2026 -SourcePath "..\plugin\bin\AddIn 2026 Release R26"
    # Install the local compiled Revit 2026 build from this repo
#>
param(
    ...
    [string]$Repo = 'mcp-servers-for-revit/mcp-servers-for-revit',
...
$scriptRootPath = if ($PSScriptRoot) { $PSScriptRoot } else { $null }
$repoRootPath = if ($scriptRootPath) {
    try {
        (Resolve-Path (Join-Path $scriptRootPath '..')).Path
    } catch {
        $null
    }
} else {
...
function Test-Prerequisites {
    ...
    if ($RequireInternet) {
        ...
        Write-Ok "Internet check skipped (using local build)"
    }
...
function Get-DefaultLocalSourcePath {
    param([string]$Year)
    ...
    $pluginBinPath = Join-Path $repoRootPath 'plugin/bin'
    if (-not (Test-Path $pluginBinPath)) {
        return $null
    }

    $preferred = Join-Path $pluginBinPath "AddIn $Year Release R$($Year.Substring(2))"
    $candidates = @()
    if (Test-Path $preferred) {
        $candidates += $preferred
    }

    $candidates += Get-ChildItem -Path $pluginBinPath -Directory -ErrorAction SilentlyContinue |
        Where-Object { $_.Name -like "AddIn $Year *" } |
        Sort-Object Name -Descending |
        Select-Object -ExpandProperty FullName

    foreach ($candidate in ($candidates | Select-Object -Unique)) {
        if ((Test-Path (Join-Path $candidate $addinFile)) -and (Test-Path (Join-Path $candidate $pluginFolder))) {
            return $candidate
        }
    }
...
function Get-DefaultLocalServerPath {
    if (-not $repoRootPath) {
        return $null
    }

    $candidate = Join-Path $repoRootPath 'server/build/index.js'
...
function Remove-ExistingInstallation {
    param([string]$AddinsPath)

    $oldAddin = Join-Path $AddinsPath $addinFile
    $oldPlugin = Join-Path $AddinsPath $pluginFolder

    if (Test-Path $oldAddin) { Remove-Item $oldAddin -Force }
    if (Test-Path $oldPlugin) { Remove-Item $oldPlugin -Recurse -Force }
}

function Test-RevitProcess {
...
function Install-FromLocalBuild {
    ...
    $manifestPath = Join-Path $BuildRoot $addinFile
    $pluginPath = Join-Path $BuildRoot $pluginFolder

    if (-not (Test-Path $manifestPath)) {
        Write-Err "Revit $Year - local build is missing $addinFile at:"
        Write-Err "  $manifestPath"
        return $false
    }

    if (-not (Test-Path $pluginPath)) {
        Write-Err "Revit $Year - local build is missing $pluginFolder at:"
        Write-Err "  $pluginPath"
        return $false
    }
    ...
    Write-Step "Revit $Year - installing local build from $BuildRoot..."
    Remove-ExistingInstallation -AddinsPath $addinsPath

    Copy-Item -Path $manifestPath -Destination (Join-Path $addinsPath $addinFile) -Force
    Copy-Item -Path $pluginPath -Destination $addinsPath -Recurse -Force
    ...
    return (Test-Installation -AddinsPath $addinsPath -Year $Year -Tag "local build")
}
...
function Set-ClaudeDesktopConfig {
    param(
        ...
        [string[]]$ServerArgs = @('/c', 'npx', '-y', 'mcp-server-for-revit')
    )
...
foreach ($ver in $targetVersions) {
    $candidate = if ($SourcePath) { $SourcePath } else { Get-DefaultLocalSourcePath -Year $ver }
    if ($candidate) {
        try {
            $resolvedLocalBuilds[$ver] = (Resolve-Path $candidate).Path
        } catch {
            Write-Err "Could not resolve local build path for Revit ${ver}:"
            Write-Err "  $candidate"
            exit 1
        }
    }
...
$needsReleaseDownload = ($targetVersions | Where-Object { -not $resolvedLocalBuilds.ContainsKey($_) }).Count -gt 0

# ── Prerequisites (only for install) ──
Write-Step "Checking prerequisites..."
Test-Prerequisites -RequireInternet:$needsReleaseDownload
Write-Host ""

# ── Fetch release info if needed ──
$release = $null
...
$installed = 0
foreach ($ver in $targetVersions) {
    if ($resolvedLocalBuilds.ContainsKey($ver)) {
        Write-Ok "Revit $ver - using local build"
        if (Install-FromLocalBuild -Year $ver -BuildRoot $resolvedLocalBuilds[$ver]) {
            $installed++
        }
    } elseif (Install-ForVersion -Year $ver -Release $release) {
        $installed++
    }
    Write-Host ""
}
...
if (-not $SkipMcpConfig) {
    $resolvedServerPath = if ($ServerPath) {
        try {
            (Resolve-Path $ServerPath).Path
        } catch {
            Write-Warn "Could not resolve local server path:"
            Write-Warn "  $ServerPath"
            $null
        }
    } else {
        Get-DefaultLocalServerPath
    }

    if ($resolvedLocalBuilds.Count -gt 0 -and $resolvedServerPath) {
        Write-Ok "Claude Desktop - using local MCP server build"
        Set-ClaudeDesktopConfig -ServerCommand 'node' -ServerArgs @($resolvedServerPath)
    } else {
        if ($resolvedLocalBuilds.Count -gt 0) {
            Write-Warn "Local Revit build installed, but no local server/build/index.js was found"
            Write-Warn "Claude Desktop will be configured to use the published npm package instead"
        }
